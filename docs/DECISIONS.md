...
| 39 | Weight each real-time posting by the seconds it covers inside its five-minute bin, a posting's interval running from the previous stamp to its own. Report every interval that ran more than a minute long. | The dispatch reruns inside a clock interval and posts irregular stamps. Duration weighting is exact when the battery holds one power level across the bin, which the decision cadence guarantees. A longer interval is a longer-lived price rather than a filled one, but the binned grid cannot show it, so the coverage report states where and for how long it occurred. |
...
| 50 | Add an optional MW band on actual power minus the forward position at every period and information node, in both `st` and the independent signed-flow `en`. `cap=None` adds no rows. | A nominally deliverable offer alone permits buying the entire position back in real time. The band measures the result when actual delivery is required, while preserving the original model as a regression baseline. |
| 51 | Replay the frozen deterministic, neutral and foresight settings at caps None, 1, 0.25 and 0 MW. Do not select a cap or re-tune a policy. | The cap is a hypothetical market rule. Its effect must be measured by re-solving both stages under that rule, not by subtracting the original accounting spread from net. |
| 52 | Under a finite cap, stop the replay if both dispatch attempts fail; retain decision 49's zero-power fallback only when the cap is None. Report the stopped setting beside the completed ones. | Zero power can breach a contracted delivery. An incomplete run must not be exported as evidence that obeys the cap. The physical target clipping in decision 43 alone no longer guarantees contractual feasibility, and on 2025-12-15 the risk-neutral policy at 0.25 MW proved it. A setting that has no result is itself a result, so it is shown rather than left out of the table. |
