...
#
# LEGEND
#   U            : one unit's offer and state      Sol : one solved outcome
...
#   _px          : a priced solve, returning the cheapest of its equally optimal prices
...
#   hl           : exact convex hull program; its balance dual is the CHP or AIC price
...
#   d            : demand per period               T,G : period and unit counts
...
#   lb,ub        : variable bounds                 d0 : demand placed on the dual objective
...
def ck(g, d):
    """Validate one market and return its filled units and demand vector."""
...
def _sy(g, d, cap, f):
    ...
    Ae = csc_array((ev, (ei, ec)), shape=(e + T, n))
    it = np.zeros(n)
    if f:
        for i in range(G):
            it[o[i] + T : o[i] + 4 * T] = 1.0
    return c, A, np.array(b), Ae, np.array(be), lb, ub, it, o
...
def uc(g, d, cap=None):
    ...
    g, d = ck(g, d)
    c, A, b, Ae, be, lb, ub, it, o = _sy(g, d, cap, True)
    res = milp(
        c,
        integrality=it,
        bounds=Bounds(lb, ub),
    ...
    p, u = _out(g, len(d), res.x, o)
    return Sol(float(res.fun), p, np.rint(u), np.full(len(d), np.nan))


def rx(g, d, cap=None):
    """Relax the same system's binaries; its balance dual is the relaxation's energy price."""
    g, d = ck(g, d)
    c, A, b, Ae, be, lb, ub, _, o = _sy(g, d, cap, False)
    res = _px(c, A, b, Ae, be, lb, ub, np.r_[np.zeros(len(g) * len(d)), d])
    if res is None:
        raise ValueError("this demand has no feasible relaxed commitment")
    p, u = _out(g, len(d), res[1], o)
    return Sol(res[0], p, u, res[2][-len(d) :])


def _px(c, A, b, Ae, be, lb=None, ub=None, d0=None):
    """Solve a priced program, then choose the cheapest of its equally optimal price vectors.

    A balance dual is a subgradient of cost in demand. At a kink, and wherever the p-cut has
    made the program integral, many prices are equally optimal and a solver returns whichever
    one its basis lands on. The second program below re-picks, among exactly those, the price
    vector that pays least for the demand: the left-hand marginal cost of what was served.
    """
    A = csc_array(A)
    Ae = csc_array(Ae)
    ...
    q = linprog(
        np.r_[np.zeros(m), d0],
        A_ub=csc_array(hstack([A.T, Ae.T])),
        b_ub=c,
        A_eq=csc_array(np.r_[b, be].reshape(1, -1)),
        b_eq=[res.fun],
        bounds=[(None, 0.0)] * m + [(None, None)] * e,
    )
    pi = q.x[m:] if q.success else res.eqlin.marginals
...
def _jt(g, d, C, j):
    ...
    for i in range(G):
        ...
        b.append(C[i][j[i]][4])
        c[i * T : (i + 1) * T] = g[i].c
    res = _px(c, np.vstack(A), np.concatenate(b), np.tile(np.eye(T), (1, G)), d, d0=d)
    if res is None:
        return None
    k = sum(C[i][j[i]][5] for i in range(G))
    return res[0] + k, res[1].reshape(G, T), res[2]
...
def en(g, d, cap=None):
    ...
    if best is None:
        raise ValueError("this demand has no feasible unit commitment")
    return Sol(best[0], best[1], best[2], np.full(T, np.nan))
...
def lmp(g, d, s, cap=None):
    ...
    res = _jt(g, d, _hold(g, T, u, cap), [0] * len(g))
    if res is None:
...
def hl(g, d, cap=None):
    ...
    for i in range(G):
        for j, (_, _, _, A, y, k) in enumerate(C[i]):
            ...
            ev.extend(np.ones(T))
    A = csc_array((va, (ri, ci)), shape=(m, n))
    Ae = csc_array((ev, (ei, ec)), shape=(G + T, n))
    res = _px(c, A, np.zeros(m), Ae, np.r_[np.ones(G), d], np.zeros(n), None,
              np.r_[np.zeros(G), d])
    if res is None:
    ...
    return Sol(res[0], p, u, res[2][G:])


def _om(g, T, pi, cap=None):
    ...
    for i, x in enumerate(g):
        ...
        lb, ub = _fx(x, T)
        res = milp(
            np.r_[x.c - pi, np.full(T, x.nl), np.full(T, x.su), np.zeros(T)],
...
def qd(g, d, pi, cap=None):
    ...
    if pi.shape != d.shape or not np.isfinite(pi).all():
        raise ValueError("pi must hold one finite price per period")
    return float(pi @ d - _om(g, len(d), pi, cap).sum())
