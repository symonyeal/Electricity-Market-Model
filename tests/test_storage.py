...
def ck(b, da, rt, pr, h, r, a=0.95, w=0.0):
    """Audit returned schedules directly, without either formulation's row builder."""
    assert np.all(r.c >= -1e-7) and np.all(r.c <= b.c + 1e-7)
    assert np.all(r.d >= -1e-7) and np.all(r.d <= b.d + 1e-7)
    assert np.max(np.minimum(r.c, r.d)) < 1e-7
    e = b.e0 + b.dt * np.cumsum(b.ec * r.c - r.d / b.ed, axis=1)
    assert r.e == pytest.approx(e, abs=1e-7)
...
def test_settlement(f):
    ...
    ck(b, [40], [[100]], [1], None, r)
    with pytest.raises(ValueError):
        f(B(1, 1, 1), [40], [[100]], [1], q=[1])
